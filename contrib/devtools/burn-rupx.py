...
"""
burn-rupx: create a bitcoin burn address
By James C. Stroud
...
Call the program with a template burn address as the only argument::
    % burn-rupx LZBurnAddressForRupxPreminexxxxxxx
    LZBurnAddressForRupxPreminextStefG
For instructions, call the program with no arguments::
    % burn-rupx
    usage: burn-rupx TEMPLATE
...
class BurnrupxError(Exception):
    pass


class AlphabetError(BurnrupxError):
    pass
...
def usage():
    print "usage: burn-rupx TEMPLATE"
    print
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        usage()
    if sys.argv[1] == "test":
        template = "RXBurnAddressForRupxPreminexxxxxxx"
    else:
        template = sys.argv[1]
    for c in template:
        if c not in ABET:
