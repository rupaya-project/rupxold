...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "rupaya.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "rupayad.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the RUPAYA and zRUPX money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in RUPAYA/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in RUPAYA/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, rupaya, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in RUPAYA/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all RUPAYA specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-rupxstake=<n>", strprintf(_("Enable or disable staking functionality for RUPAYA inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zrupxstake=<n>", strprintf(_("Enable or disable staking functionality for zRUPX inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzrupx=<n>", strprintf(_("Enable automatic wallet backups triggered after each zRUPX minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zrupxbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zRUPX backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizerupayaamount=<n>     " + strprintf(_("Keep N RUPAYA anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Rupaya Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that RUPAYA is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Rupaya Core is shutting down."));
...
#endif
    // Make sure only a single RUPAYA process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Rupaya Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //RUPAYA: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //RUPAYA specific: zerocoin and spork DB's
                ...
                // RUPAYA: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Rupaya Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Rupaya Core to complete") << "\n";
        ...
        zwalletMain = new CzRUPXWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zRUPXWallet
        uiInterface.InitMessage(_("Syncing zRUPX wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZRupxBackups = GetBoolArg("-backupzrupx", true);
        pwalletMain->setZRupxAutoBackups(fEnableZRupxBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zrupxTracker->Init();
...
//    }
//
//    nAnonymizeRupayaAmount = GetArg("-anonymizerupayaamount", 0);
//    if (nAnonymizeRupayaAmount > 999999) nAnonymizeRupayaAmount = 999999;
//    if (nAnonymizeRupayaAmount < 2) nAnonymizeRupayaAmount = 2;
    ...
    LogPrintf("Anonymize RUPAYA Amount %d\n", nAnonymizeRupayaAmount);
    ...
    /* Denominations
       ...
       For example:
       1RUPAYA+1000 == (.1RUPAYA+100)*10
       10RUPAYA+10000 == (1RUPAYA+1000)*10
    */
