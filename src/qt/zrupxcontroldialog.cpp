...
std::set<std::string> ZRupxControlDialog::setSelectedMints;
std::set<CMintMeta> ZRupxControlDialog::setMints;

bool CZRupxControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZRupxControlDialog::COLUMN_DENOMINATION || column == ZRupxControlDialog::COLUMN_VERSION || column == ZRupxControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZRupxControlDialog::ZRupxControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZRupxControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZRupxControlDialog::~ZRupxControlDialog()
{
    delete ui;
}

void ZRupxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZRupxControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZRupxControlWidgetItem* itemDenom(new CZRupxControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZRupxControlWidgetItem *itemMint = new CZRupxControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZRupxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZRupxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZRupx_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZRupxControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZRupxControlDialog::GetSelectedMints()
{
...
void ZRupxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
