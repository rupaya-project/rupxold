...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered rupaya address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 RUPX.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal rupaya:URI (rupaya:XyZ...?message=example)
            {
...
static void NotifyzRUPXReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzRUPXReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzRUPXReset.connect(boost::bind(NotifyzRUPXReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzRUPXReset.disconnect(boost::bind(NotifyzRUPXReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zrupxTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
