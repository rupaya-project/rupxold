                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zRUPX ought to be enough for anybody." - Bill Gates, 2017
    ui->zRUPXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzRUPXSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zRUPXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzRUPX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zRUPX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zRUPX, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zRUPX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zRUPX in ") +
...
void PrivacyDialog::on_pushButtonSpendzRUPX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zRUPX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zRUPX, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zRUPX
        sendzRUPX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zRUPX
    sendzRUPX();
}

void PrivacyDialog::on_pushButtonZRupxControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZRupxControlDialog* zRupxControl = new ZRupxControlDialog(this);
    zRupxControl->setModel(walletModel);
    zRupxControl->exec();
}

void PrivacyDialog::setZRupxControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzRupxSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzRUPX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Rupaya Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zRUPXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zRUPX is requested
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zRUPX</b>";
    ...
    // use mints from zRUPX selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZRupxControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZRupxControlDialog::GetSelectedMints();
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zRUPX was spent successfully update the addressbook with the label
    ...
    // Clear zrupx selector in case it was used
    ZRupxControlDialog::setSelectedMints.clear();
    ui->labelzRupxSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zRUPX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Rupx, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zRUPX Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zrupxTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zRUPX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zRUPX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zRUPX "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zRUPX "));
    ui->labelzRUPXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zRUPX </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zRUPX </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zRUPX </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzRUPXSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzRUPX->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zRUPX
        ui->pushButtonMintzRUPX->setEnabled(false);
        ui->pushButtonMintzRUPX->setToolTip(tr("zRUPX is currently disabled due to maintenance."));

        // Spend zRUPX
        ui->pushButtonSpendzRUPX->setEnabled(false);
        ui->pushButtonSpendzRUPX->setToolTip(tr("zRUPX is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zRUPX
        ui->pushButtonMintzRUPX->setEnabled(true);
        ui->pushButtonMintzRUPX->setToolTip(tr("PrivacyDialog", "Enter an amount of RUPAYA to convert to zRUPX", 0));

        // Spend zRUPX
        ui->pushButtonSpendzRUPX->setEnabled(true);
        ui->pushButtonSpendzRUPX->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
