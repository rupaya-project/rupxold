...
bool CZRupxStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
    ...
    CAmount zRupxToMint = nTotal - masternodeReward;

    LogPrintf("%s: Total=%d Masternode=%d Staker=%d\r\n", __func__, (nTotal / COIN), (masternodeReward / COIN), (zRupxToMint / COIN));   

    std::map<libzerocoin::CoinDenomination, int> mintMap = calculateOutputs(zRupxToMint);
