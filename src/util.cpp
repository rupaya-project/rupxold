...
int nAnonymizeRupayaAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "rupaya" is a composite category enabling all RUPAYA-related debug output
            if (ptrCategory->count(string("rupaya"))) {
...
#else
    const char* pszModule = "rupayacore";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "rupaya.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty rupaya.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override rupaya.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "rupayad.pid"));
