...
CzRUPXWallet::CzRUPXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zrupx seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZRUPXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZRUPXSeed_deprecated()) {
                    LogPrintf("%s: Updated zRUPX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zrupx seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zrupx wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzRUPXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZRUPXCount(nCountLastUsed);
    else if (!walletdb.ReadZRUPXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzRUPXWallet::Lock()
{
    seedMaster = 0;
}

void CzRUPXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzRUPXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZRUPX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzRUPXWallet::LoadMintPoolFromDB()
{
...
void CzRUPXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzRUPXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzRUPXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zrupxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzRUPXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZRUPX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zrupxTracker which also adds to database
    pwalletMain->zrupxTracker->Add(dMint, true);
...
void CzRUPXWallet::SeedToZRUPX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzRUPXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzRUPXWallet::GenerateDeterministicZRUPX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzRUPXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzRUPXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
