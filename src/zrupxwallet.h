...
#ifndef RUPAYA_ZRUPXWALLET_H
#define RUPAYA_ZRUPXWALLET_H
...
public:
    CzRUPXWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZRUPX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZRUPX(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //RUPAYA_ZRUPXWALLET_H
